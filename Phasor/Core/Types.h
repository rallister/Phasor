...
{
	ident map_id; // 0x0000 ModelTag id.
	UNKNOWN(12);
	UNKNOWN_BITFIELD(2);
	...
	float shield; // 0x00e4
	UNKNOWN(0x10);
	vect3d locationInVehicle; // 0x00f8 set when in a vehicle unlike other one. best not to use tho (isnt always set)
	UNKNOWN(0x10);
...
struct s_player_structure
{
	...
	DWORD	lastBulletShotTime;				// since game start(0)
	...
	UNKNOWN(18);
	WORD	flagStealCount;
	WORD	flagReturnCount;
	WORD	flagCaptureCount;
	UNKNOWN(6);
	ident unknownId4;
	UNKNOWN(8);
	WORD	ping;
